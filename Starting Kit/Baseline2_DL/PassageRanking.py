...
# The following LoadValidationSet method reads ctf format validation file and creates query, passage feature vectors
# and also copies labels for each pair. The created vectors will be useful to find metrics on validation set after
# training each epoch which will be useful to decide the best model
...
# The following method defines a CNN network which has series of convolution and max pooling steps on query features
# and passage features and then a merge step and it follows a fully connected layer
def cnn_network(queryfeatures, passagefeatures, num_classes):
    with C.layers.default_options(activation=C.ops.relu, pad=False):
        convA1 = C.layers.Convolution2D((3, 10), 4, pad=False, activation=C.tanh, name='convA1')(
            queryfeatures)  # input : 12*50 #output : 4*10*41
        poolA1 = C.layers.MaxPooling((2, 3), (2, 3), name='poolA1')(convA1)  # output : 4*5*13
        convA2 = C.layers.Convolution2D((2, 4), 2, pad=False, activation=C.tanh, name='convA2')(
            poolA1)  # output : 2*4*10
        ...
        convB1 = C.layers.Convolution2D((5, 10), 4, pad=False, activation=C.tanh, name='convB1')(
            passagefeatures)  # input : 50*50  #output : 4*46*41
        poolB1 = C.layers.MaxPooling((5, 5), (5, 5), name='poolB1')(convB1)  # output : 4*9*8
        convB2 = C.layers.Convolution2D((3, 3), 2, pad=False, activation=C.tanh, name='convB2')(
            poolB1)  # output : 2*7*6
...
def create_reader(path, is_training, query_total_dim, passage_total_dim, label_total_dim):
    return MinibatchSource(CTFDeserializer(path, StreamDefs(
        queryfeatures=StreamDef(field='qfeatures', shape=query_total_dim, is_sparse=False),
...
def TrainAndValidate(trainfile):
    ...
    input_map = {query_input_var: train_reader.streams.queryfeatures,
                 passage_input_var: train_reader.streams.passagefeatures, output_var: train_reader.streams.labels}
    ...
    for epoch in range(total_epochs):  # loop over epochs
        ...
        while sample_count < epoch_size:  # loop over minibatches in the epoch
            data = train_reader.next_minibatch(min(minibatch_size, epoch_size - sample_count),
        ...
        # *** Find metrics on validation set after every epoch ******#  (Note : you can skip doing this for every
        # epoch instead to optimize the time, do it after every k epochs)
        predicted_labels = []
        ...
        metrics = precision_recall_fscore_support(np.array(validation_labels), np.array(predicted_labels),
                                                  average='binary')
...
# The following GetPredictionOnEvalSet method reads all query passage pair vectors from CTF file and does
# forward prop with trained model to get similarity score after getting scores for all the pairs, the output will
# be written into submission file.
